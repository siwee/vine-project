package io.github.aomsweet.petty.app.graal;
    ...
    public static boolean isAlpnSupported(final SslProvider provider) {
        switch (provider) {
            ...
            case OPENSSL_REFCNT:
                return false;
            default:
                throw new Error("SslProvider unsupported on Petty " + provider);
        }
    }
}
...
final class Target_io_netty_handler_ssl_OpenSsl {
    ...
    private static Throwable UNAVAILABILITY_CAUSE = new RuntimeException("OpenSsl unsupported on Petty");
